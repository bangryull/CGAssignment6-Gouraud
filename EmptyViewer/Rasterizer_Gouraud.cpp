...
void rasterize_triangle(const vec4& v0, const vec4& v1, const vec4& v2, const ivec3& tri) {
    ...
    vec3 c0 = compute_lighting(wp0, vertex_normals[tri.x]);
    vec3 c1 = compute_lighting(wp1, vertex_normals[tri.y]);
    vec3 c2 = compute_lighting(wp2, vertex_normals[tri.z]);
        ...
        for (int x = minX; x <= maxX; ++x) {
            ...
            if (inside_triangle(p, p0, p1, p2, bary)) {
                ...
                if (z < depthBuffer[idx]) {
                    depthBuffer[idx] = z;
                    vec3 color = bary.x * c0 + bary.y * c1 + bary.z * c2;
